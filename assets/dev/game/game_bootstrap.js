/* game_bootstrap.js — robust start flow with player rig integration
   ...
   - Safe HUD + weather initialization
*/
(function () {
  "use strict";
  ...
  function createEngineScene() {
    ...
    camera.minZ = 0.1;
    camera.inputs.clear(); // disable default Babylon inputs
  ...
  function enforceSpawn() {
    ...
    if (window.PP?.rig?.body) {
      try { window.PP.rig.body.position.set(sp.x, sp.y, sp.z); } catch { }
    } else {
      const capsule = BABYLON.MeshBuilder.CreateCapsule("playerCapsule", { height: 1.8, radius: 0.25 }, scene);
      capsule.position.set(sp.x, sp.y, sp.z);
      capsule.checkCollisions = true;
      capsule.isVisible = false;
      window.PP = window.PP || {};
      window.PP.rig = window.PP.rig || {};
      window.PP.rig.body = capsule;
      scene.registerBeforeRender(() => {
        if (window.PP.rig.body.position.y < -10) window.PP.rig.body.position.set(sp.x, sp.y, sp.z);
      });
    }
  }
  ...
  async function loadPlayerRig() {
    Loader.addStep("Loading player rig…", async () => {
      ...
      await new Promise(resolve => {
        ...
        document.addEventListener("pp:rig-ready", () => { clearTimeout(t); resolve(); }, { once: true });
      });
    });
  }
  ...
  async function startGame() {
    ...
    Loader.show();
    Loader.reset();
    mark("start-button-click");

    try { window.PP?.audio?.init?.(); } catch { }

    document.getElementById("title-screen")?.remove();
    const canvas = $("#renderCanvas");
    canvas?.requestPointerLock?.(); canvas?.focus?.();

    // engine + scene
    Loader.addStep("Creating scene…", () => createEngineScene());
    ...
    // finalize weather
    document.dispatchEvent(new CustomEvent("pp:start"));
  }
