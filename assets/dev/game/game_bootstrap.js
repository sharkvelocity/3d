/* game_bootstrap.js — full bootstrap with map loader, player rig, ghost system, and PS5 support */
(function () {
"use strict";
...
// ---------- Ghosts + PS5 controller injection ----------
async function injectGhostsAndPS5() {
    const ghostsLoaded = await loadScriptOnce("./assets/dev/ghost/ghost_data.js");
    if (!ghostsLoaded) warn("ghost_data.js failed to load, ghost features may be disabled");

    const ps5Loaded = await loadScriptOnce("./assets/dev/ui/ps5_controller.js");
    if (!ps5Loaded) warn("ps5_controller.js failed to load, controller input may be limited");

    if (window.GhostSystem?.init) window.GhostSystem.init();
    if (window.PS5Controller?.init) window.PS5Controller.init();
}
...
async function loadMap(mapData){
    ...
    if(mapData.file.toLowerCase().endsWith(".glb")){
        ...
        if (window.MapGenerator) {
            const currentMapGen = window.MapGenerator;
            await currentMapGen.spawnRooms();
            unified.rooms = currentMapGen.rooms || [];
            unified.doors = currentMapGen.doors || [];
            unified.meshes = currentMapGen.roomMeshes || [];
            const van = currentMapGen.getVanRoom && currentMapGen.getVanRoom();
            if(van) unified.spawn = new BABYLON.Vector3(van.position.x, 1.8, van.position.z);
        }
    }
    ...
    if(window.spawnPlayer) window.spawnPlayer();
...
async function startGame(){
    ...
    try {
        ...
        Loader.addStep("Injecting ghost & PS5 support…", async () => injectGhostsAndPS5());
        ...
        Loader.addStep("Finalizing…", async () => {
            ...
            if(typeof window.enablePointerLockOnce==="function") window.enablePointerLockOnce();
        });
