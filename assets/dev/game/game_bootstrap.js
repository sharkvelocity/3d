/* game_bootstrap.js — robust start flow with player rig integration
   ...
   - Fully integrates player rig (WASD, PS5, camera, animations, physics)
*/
(function () {
"use strict";
...
function createEngineScene(){
  ...
  // Camera placeholder; will attach to rig
  camera = new BABYLON.UniversalCamera("playerCam", new BABYLON.Vector3(0,1.8,0), scene);
  camera.minZ = 0.1;
  camera.inputs.clear(); // disable default controls
    ...
    Loader.addStep("Initializing player physics & movement…", async () => {
      ...
      if (window.PP?.rig?.controller?.enable) {
        ...
        scene.registerBeforeRender(() => {
          const speed = 0.12;
          if(!body) return;
          if(keys.w) body.moveWithCollisions(new BABYLON.Vector3(0,0,speed));
          if(keys.s) body.moveWithCollisions(new BABYLON.Vector3(0,0,-speed));
          if(keys.a) body.moveWithCollisions(new BABYLON.Vector3(-speed,0,0));
          if(keys.d) body.moveWithCollisions(new BABYLON.Vector3(speed,0,0));
        });
      }
    });
