/* game_bootstrap.js — full bootstrap with map loader, player rig, ghost integration, and PS5 support */
(function(){
"use strict";
...
function applyMapDefToRoot(root){
    ...
    try{
        ...
        root.rotation.set(0,(d.rotationY||0)*Math.PI/180,0);
...
async function loadMap(mapData){
    ...
    if(mapData.file.toLowerCase().endsWith(".glb")){
        ...
        unified.type="glb"; unified.meshes=res.meshes;
        unified.spawn=mapData.spawn ? new BABYLON.Vector3(mapData.spawn.x,mapData.spawn.y,mapData.spawn.z) : new BABYLON.Vector3(0,1.8,0);
        ...
        if(window.MapGenerator){
            const g=window.MapGenerator;
            await g.spawnRooms();
            unified.rooms=g.rooms||[];
            unified.doors=g.doors||[];
            unified.meshes=g.roomMeshes||[];
            const van = g.getVanRoom && g.getVanRoom();
    ...
    if(window.GhostSystem?.onMapLoaded) GhostSystem.onMapLoaded(unified, scene);
...
async function injectGhostsAndPS5(){
    await loadScriptOnce("./assets/ghost_data.js");
    await loadScriptOnce("./assets/dev/util/ps5_controller_phasmaphobia.js");
}
...
async function startGame(){
    ...
    try{
        ...
        Loader.addStep("Finalizing…", async()=>{
            ...
            if(typeof enablePointerLockOnce==="function") enablePointerLockOnce();
        });
